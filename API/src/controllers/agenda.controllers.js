...
async function verifyUcuWorker(ci) {
  // Esta función debe ser asincrónica ya que debe hacer una consulta a la tabla de login
  const connection = await pool.getConnection();
  const [result] = await connection.execute(
    "SELECT COUNT(*) AS count FROM funcionariosUcu WHERE logId = ?",
    [logId]
  );
  connection.release();
  return result[0].count === 0;
...
export const getFecha = async (req, res) => {
  try {
    ...
    const [result] = await connection.execute(
      "SELECT fch_agenda FROM agenda WHERE ci = ?;",
      [req.body.ci]
    );
...
export const addFecha = async (req, res) => {
  try {
    ...
    try {
      ...
      if (!ciExiste) {
        return res.status(400).json({
          error: "No existe un usuario registrado con esta CI.",
        });
      }
      ...
      const [result] = await connection.execute(
        ...
        [req.body.ci, req.body.fch_agenda]
      );
      ...
      res.status(201).json({
        id: result.insertId,
        mensaje: "Fecha agregada correctamente.",
      });
    } catch (error) {
      return res
        .status(400)
